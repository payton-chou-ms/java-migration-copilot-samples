/**
 * IosFrame — iPhone 設備邊框
 * 參考 iPhone 15 Pro（393×852 logical pixels）
 * 含：灵动岛 + 狀態欄（時間/訊號/電池）+ Home Indicator + 圓角
 *
 * 用法（在 <script type="text/babel"> 內使用）：
 ...
 * Props:
 *   width       {number}  - 默認 393
 *   height      {number}  - 默認 852
 *   time        {string}  - 默認 '9:41'
 *   battery     {number}  - 0-100，默認 100
 *   darkMode    {boolean} - 默認 false
 *   showStatusBar       {boolean} - 默認 true
 *   showDynamicIsland   {boolean} - 默認 true
 *   showHomeIndicator   {boolean} - 默認 true
 */
...
if (typeof window !== 'undefined') {
  ...
  window.iosFrameStyles = iosFrameStyles;
}
